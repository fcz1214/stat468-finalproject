...
# Initialize DuckDB with S3 data
def load_players_from_s3():
    try:
        # Used for debug
        ...
        if credentials:
            ...
            db.execute(f"SET s3_access_key_id='{credentials.access_key}'")
            db.execute(f"SET s3_secret_access_key='{credentials.secret_key}'")
        
        # Load data from S3
        s3_path = "s3://tennis-cfu-2024/data/atp_tennis.csv"
        db.execute(f"CREATE TABLE tennis AS SELECT * FROM read_csv_auto('{s3_path}') WHERE Rank_1 <= 50 AND Rank_1 > 0")
        
        # Get top 30 players with their most recent rankings
        query = """
        SELECT Player_1 as player, MIN(Rank_1) as best_rank, COUNT(*) as matches
        FROM tennis 
        GROUP BY Player_1
        HAVING best_rank <= 30
        ORDER BY best_rank
        ...
        players_df = db.execute(query).fetchdf()
        
        # Convert to the format your app expects
        players_dict = {}
        for _, row in players_df.iterrows():
            players_dict[row['player']] = {
                "rank": int(row['best_rank']),
                "country": "International"  # Simplified for now
            }
        
        print(f"Loaded {len(players_dict)} players from S3 via DuckDB")
        return players_dict
        
    except Exception as e:
        print(f"Error loading from S3: {e}")
        # Fallback to your original hardcoded players use the same code from app.py
        return {
...
# Load players from S3
players = load_players_from_s3()
...
# Application UI (keeping your exact styling)
app_ui = ui.page_fluid(
    ...
    ui.div(
        ...
        ui.p("Professional tennis match outcome predictions powered by S3 data and DuckDB"),
        class_="header"
    ),
    ...
    ui.page_sidebar(
        ui.sidebar(
            ui.div(
                ...
                ui.div(
                    ui.p("📊 Data Source: AWS S3", style="color: #666; font-size: 0.7em; margin: 0;"),
                    ui.p("🔍 Engine: DuckDB", style="color: #666; font-size: 0.7em; margin: 0;")
                ),
    ...
    def connection_status():
        if check_api_connection():
            return ui.div(
                ...
                ui.p("ML Model + S3 Active", style="color: #666; font-size: 0.7em; margin: 0;")
            )
        else:
            return ui.div(
                ui.p("🔴 API: Offline", style="color: red; margin: 0; font-weight: bold; font-size: 0.8em;"),
                ui.p("S3 Fallback Mode", style="color: #666; font-size: 0.7em; margin: 0;")
            )
